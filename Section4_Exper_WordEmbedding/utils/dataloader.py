...
def get_dataloader(model_name, vocab, data_dir, batch_size, shuffle, filter):
    """
    ...
    # 加载单一文件
    # cbow_input, cbow_output, skipgram_input, skipgram_output \
    #     = load_and_prepare_data(model_name, os.path.join(data_dir, "test.txt"), vocab, filter)
    ...
    for file_name in os.listdir(data_dir):
        ...
        if os.path.isfile(file_path):
            ...
            file_cbow_input, file_cbow_output, file_skipgram_input, file_skipgram_output \
                = load_and_prepare_data(model_name, file_path, vocab, filter)
...
def prepare_data_CBOW(context_lines, vocab):
    """
    根据上下文生成 CBOW 模型的输入输出样本。
    :param context_lines: 节点上下文数据，每行格式如：<NL>B3D6 <L>B3C4 <L>C5D6
    :param vocab: 词汇表
    :return: CBOW 的输入输出样本
    """
    cbow_input, cbow_output = [], []
    ...
    for line in context_lines:
        ...
        cbow_input.append(nodes[1:])  # 上下文作为输入
        cbow_output.append(nodes[0])  # 当前节点作为输出

    return cbow_input, cbow_output, [], []


def prepare_data_SkipGram(context_lines, vocab):
    """
    根据上下文生成 SkipGram 模型的输入输出样本。
    :param context_lines: 节点上下文数据，每行格式如：<NL>B3D6 <L>B3C4 <L>C5D6
    :param vocab: 词汇表
    :return: SkipGram 的输入输出样本
    """
    skipgram_input, skipgram_output = [], []
    ...
    for line in context_lines:
        ...
        # 该段内容为 SkipGram 的训练过程，需要为所有的节点构建映射关系，而不区分<NL>或<L>
        skipgram_input.append(nodes[0])  # 当前节点作为输入
        skipgram_output.append(nodes[1:])  # 上下文作为输出

    return [], [], skipgram_input, skipgram_output


def prepare_data_combine(context_lines, vocab):
    """
    ...
    for line in context_lines:
        ...
        # 使用Skip-Gram：用非叶节点推导上下文，即 [<NL>C2D3]预测[<L>C2D3] 和 [<L>C2E6]
        # 使用CBOW：用上下文推导叶节点，用 [<L>C2D3] 预测 [<NL>C2D3]
        if "<NL>" in nodes[0]:
            # 将节点映射为词汇表中的索引
            nodes = [map_to_vocab(node, vocab) for node in nodes]
            skipgram_input.append(nodes[0])  # 当前节点作为输入
...
def load_and_prepare_data(model_name, file_path, vocab, filter=True):
    """
    从文件中读取节点上下文，并生成训练数据。
    :param model_name: 模型的名称（cbow, skipgram, combined）
    ...
    if filter is False:
        if model_name == "cbow":
            cbow_input, cbow_output, skipgram_input, skipgram_output \
                = prepare_data_CBOW(context_lines, vocab)
        elif model_name == "skipgram":
            cbow_input, cbow_output, skipgram_input, skipgram_output \
                = prepare_data_SkipGram(context_lines, vocab)
        else:
            raise ValueError("Choose model_name from: cbow, skipgram, combined")
            return
    else:
        cbow_input, cbow_output, skipgram_input, skipgram_output \
            = prepare_data_combine(context_lines, vocab)
